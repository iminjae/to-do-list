...
const TodoCard: FC<TodoCardProps> = ({ todo, todos, setTodos }) => {
    ...
    const onClickIsDone = () => {
        const temp = todos.map((v) => {
            if (v.id === todo.id) {
                return { id: todo.id, content: todo.content, isDone: !true };
            } else {
                return v;
            }
        });
        ...
        <Flex bgColor="gray.50" px={2} py={2} rounded="lg" gap={1}>
            {isUpdateOpen ? (
                <Flex>
                    ...
                    <Button colorScheme="purple" onClick={onClickUpdate}><FiEdit3 /></Button>
                </Flex>
            ) : (
                <Text
                    fontSize={20}
            ...
            <Button colorScheme="red" onClick={onClickDelete}><FiTrash2 /></Button>
        </Flex>
    );
};
